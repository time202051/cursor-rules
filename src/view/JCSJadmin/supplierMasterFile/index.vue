...
export default {
  name: "supplierMasterFileManagement",
  mixins: [minxin],
  mounted() {
    this.getTable();
    this.getSelect();
  ...
  methods: {
    ...
    delete() {
      ...
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        .then(() => {
          this.del({
            url: warehouse.supplier + "/" + data[0].id,
          }).then((res) => {
            this.getTable();
